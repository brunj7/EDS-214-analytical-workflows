...
For this session, we are goin to select the `RStudio` option and hit `Start Session`.
...
A user can also installed her/his own packages. Let's try to install the `remote` package that lets you install R packages directly from GitHub: `install.packages("remotes")`. Once done, note a new section that appeared on the Packages tab named `User Library`. Each of us have now its own copy ot the package installed (in this `R` folder we were talking about a few minutes ago). 
...
* List files showing ownership and permissions: `ls -l`
			...
			brun@taylor:~/postdoc-training/data$ ls -l
			total 1136
...
Here is an example of my uploading the file `10min-loop.R` to taylor from my laptop. The destination directory on taylor is `/home/brun/github_com/NCEAS/nceas-training/materials/files`:

```bash
scp 10min-loop.R brun@taylor.bren.ucsb.edu:/home/brun/github_com/NCEAS/nceas-training/materials/files
```
